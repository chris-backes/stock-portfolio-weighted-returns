...
const WeightedReturn = ({ portfolio }) => {
	...
	const spyWeightedReturnAnnualized =
		((spyCurrAmount / spyWeightedAmount) ** (365 / averageWeighted) * 100) - 100;
	const nasdaqWeightedReturnAnnualized =
		((nasdaqCurrAmount / nasdaqWeightedAmount) ** (365 / averageWeighted) *
		100) - 100;
	const spyWeightedReturnTotal = ((spyCurrAmount / spyWeightedAmount) * 100) - 100;
	const nasdaqWeightedReturnTotal =
		((nasdaqCurrAmount / nasdaqWeightedAmount) * 100) - 100;
	
	const assessment = () => {
		const investingPeriod = (new Date(new Date().setDate(new Date().getDate() - 1)) - new Date(absoluteStart)) / (1000 * 3600 * 24 * 365) //converts the milliseconds to years with decimals
		if (weightedReturn > spyWeightedReturnAnnualized && weightedReturn > nasdaqWeightedReturnAnnualized) {
		...
		} else if (weightedReturn < spyWeightedReturnAnnualized && weightedReturn < nasdaqWeightedReturnAnnualized) {
			if (investingPeriod > 5) {
	...
	return (
		<section>
			...
			<p>
				...
				this average length is {weightedReturn.toFixed(2)}%.
			</p>
			<p>
				Over the equivalent weighted period, the S&P has returned{" "}
				{spyWeightedReturnTotal.toFixed(2)}%, for an annualized
				return of {spyWeightedReturnAnnualized.toFixed(2)}%.
				The Nasdaq has returned{" "}
				{nasdaqWeightedReturnTotal.toFixed(2)}%, for an
				annualized return of{" "}
				{nasdaqWeightedReturnAnnualized.toFixed(2)}%.
			</p>
			<p>{assessment()}</p>
			<Explanation />
		</section>
